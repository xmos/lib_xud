...
def do_test(arch, tx_clk, tx_phy, seed):
    ...
    packets.append(
        RxDataPacket(rand, data_start_val=dataval, length=10, pid=0x3)
    )  # DATA0
    ...
    packets.append(
        RxDataPacket(rand, data_start_val=dataval, length=11, pid=0xB)
    )  # DATA1
    packets.append(TxHandshakePacket())
    ...
    AppendInToken(packets, ep, inter_pkt_gap=5000)
    packets.append(
        RxDataPacket(rand, data_start_val=dataval, length=12, pid=0x3)
    )  # DATA0
    # packets.append(TxHandshakePacket())
    ...
    AppendInToken(packets, ep, inter_pkt_gap=5000)
    packets.append(
        RxDataPacket(rand, data_start_val=dataval, length=13, pid=0xB)
    )  # DATA1
    packets.append(TxHandshakePacket())
    ...
    AppendInToken(packets, ep, inter_pkt_gap=5000)
    packets.append(
        RxDataPacket(rand, data_start_val=dataval, length=14, pid=0x3)
    )  # DATA0
    packets.append(TxHandshakePacket())

    # Note, quite big gap to allow checking.
    do_rx_test(
        arch, tx_clk, tx_phy, packets, __file__, seed, level="smoke", extra_tasks=[]
    )
