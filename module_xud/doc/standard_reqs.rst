...
Since these requests are common across most (if not all) devices, a some useful
...
and parses it into a SetupPacket structure for further inspection (A SetupPacket
structure is passed by reference into the ``USB_GetSetupPacket()`` call, which is
populated by the function).  This structure closely matches the structure defined
...
does not guarantee a fully USB compliance device.  Each request could well be required
...
The function inspects this SetupPacket structure and includes a minimum implementation of the
...
- ``SET_INTERFACE``
    ...
    - A global variable is maintained for each interfaces, this is updated by a ``SET_INTERFACE``.
...
marked "Halted" (Using ``SetStall_Out()`` and ``SetStall_In()``) and the function returns 1.
...
the SetupPacket for any class/applicaton specific requests. Then makes a call to ``USB_StandardRequests()`` eg:   
