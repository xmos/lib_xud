...
if platform_is_windows():
  xsi_lib_path = os.path.abspath(os.environ['XCC_EXEC_PREFIX'] + '../lib/xsidevice.dll')
else:
  ...
  def _wait_for_port_pins_change(self, ps):
    ...
    def _resume(xsi):
      for i in range(len(ps)):
        (package, pin, _), (tile, p, bit, mask) = ps[i]
...
class Xsi(object):
    ...
    def sample_port_pins(self, tile, port, mask):
      tile = tile.encode('utf-8')
      port = port.encode('utf-8')
      c_tile = c_char_p(tile)
      c_port = c_char_p(port)
      c_mask = c_int(mask)
      c_value = c_int()
      status = xsi_lib.xsi_sample_port_pins(self.xsim, c_tile, c_port,
    ...
    def drive_port_pins(self, tile, port, mask, value):
      ...
      c_mask = c_int(mask)
      c_value = c_int(value)
      status = xsi_lib.xsi_drive_port_pins(self.xsim, c_tile, c_port,
                                            c_mask, c_value)
      XsiStatus.error_if_not_valid(status)

    # TOOD make this pin*s*
    def drive_periph_pin(self, periph, pin, mask, value):
      periph = periph.encode('utf-8')
      pin = pin.encode('utf-8')
      c_periph = c_char_p(periph)
      c_pin = c_char_p(pin)
      c_mask = c_int(mask)
      c_value = c_int(value)
      status = xsi_lib.xsi_drive_periph_pin(self.xsim, c_periph, c_pin,
                                            c_mask, c_value)
      XsiStatus.error_if_not_valid(status)
    ...
    def is_port_pins_driving(self, tile, port):
      ...
      c_port = c_char_p(port)
      c_value = c_int()
      status = xsi_lib.xsi_is_port_pins_driving(self.xsim, c_tile, c_port,
                                                byref(c_value))
      XsiStatus.error_if_not_valid(status)
      return c_value.value
    ...
    def read_mem(self, tile, address, num_bytes, return_ctype=False):
      ...
      status = xsi_lib.xsi_read_mem(self.xsim, c_tile, c_address,
                                    c_num_bytes, buf)
      XsiStatus.error_if_not_valid(status)
      if return_ctype:
        return buf
    ...
    def write_mem(self, tile, address, num_bytes, data):
      ...
      status = xsi_lib.xsi_write_mem(self.xsim, c_tile, c_address,
                                    c_num_bytes, buf)
      XsiStatus.error_if_not_valid(status)
    ...
    def read_pswitch_reg(self, tile, reg_num):
      ...
      c_value = c_int()
      status = xsi_lib.xsi_read_pswitch_reg(self.xsim, c_tile, c_reg_num,
                                            byref(c_value))
      XsiStatus.error_if_not_valid(status)
      return c_value.value
    ...
    def write_pswitch_reg(self, tile, reg_num, value):
      ...
      c_reg_num = c_int(reg_num)
      c_value = c_int(value)
      status = xsi_lib.xsi_write_pswitch_reg(self.xsim, c_tile, c_reg_num,
                                            c_value)
      XsiStatus.error_if_not_valid(status)
