...
def do_test(arch, clk, phy, data_valid_count, usb_speed, seed, verbose=False):
    ...
    for pktLength in range(start_length, end_length+1):
        session.add_event(UsbTransaction(session, deviceAddress=address, endpointNumber=ep_loopback, endpointType="ISO", direction= "OUT", dataLength=pktLength))
        ...
        session.add_event(UsbTransaction(session, deviceAddress=address, endpointNumber=ep_loopback, endpointType="ISO", direction= "IN", dataLength=pktLength, interEventDelay=498))
